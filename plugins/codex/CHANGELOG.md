...
- `/codex:adversarial-review` moves to `skills/adversarial-review/SKILL.md`, stays model-invokable, and always backgrounds. Its `--wait`/`--background` flags were decoration and are gone from the docs entirely: `handleReviewCommand` parses both and then calls `runForegroundCommand` unconditionally, so only a Claude-side background task ever detached a review. Dropping them removes the size estimate and the `AskUserQuestion` round trip before every challenge review; the emptiness guard stays, because `auto` scope resolves to a branch diff on a clean tree and would otherwise spend a run reviewing nothing.
